...
CREATE SERVER "stripe_server" FOREIGN DATA WRAPPER "stripe" OPTIONS (
    "api_key_id" '9c9188a1-1127-4546-9689-990e5cc88aa3',
...
CREATE OR REPLACE TRIGGER "reward_confirmation_request_notification" AFTER UPDATE ON "public"."transaction_units" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://wxcvnmqyajozzqhzfnlp.supabase.co/functions/v1/rewardConfirmationRequestNotification', 'POST', '{"Content-type":"application/json"}', '{}', '5000');

CREATE OR REPLACE TRIGGER "transaction_completed_notification" AFTER UPDATE ON "public"."transactions" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://wxcvnmqyajozzqhzfnlp.supabase.co/functions/v1/transactionCompletedNotification', 'POST', '{"Content-type":"application/json"}', '{}', '5000');
